"""
Glacier SCF Gap-Filling via XGBoost
=====================================
Trains an XGBoost regressor on land pixels (observed SCF + terrain
features) and predicts SCF for glacier-masked pixels, producing daily
gap-filled snow cover maps.

XGBoost differs from Random Forest in that trees are built sequentially —
each new tree corrects the residual errors of the previous ones (gradient
boosting). This usually achieves lower error than RF at the same number of
trees, but is more sensitive to hyperparameter choices (learning rate,
depth, subsampling).

Features
--------
  elevation, slope, sin(aspect), cos(aspect),
  easting, sin(2π·DOY/365), cos(2π·DOY/365), year
...
Outputs
-------
  netcdf/{site}_filled_xgb/{site}_scf_{year}_filled_xgb.nc  — one per year
  figures/{site}/xgb_validation_scatter.png
  figures/{site}/xgb_validation_rmse_by_elevation.png
  results/csvs/{site}/xgb_validation_stats.csv

Usage
-----
  python scripts/fill_glacier_scf_xgb.py --site zackenberg
"""
...
MAX_TRAIN_SAMPLES = 500_000   # cap for XGB training after accumulation
...
# Unlike RF, XGBoost uses shallow trees (max_depth 4–6) and compensates by
# using more of them with a small learning_rate. subsample and colsample_bytree
# add randomness similar to RF's bootstrap, helping generalisation.
XGB_PARAMS = dict(
    n_estimators      = 500,    # number of boosting rounds (trees)
    max_depth         = 5,      # shallow trees — XGBoost uses many small corrections
    learning_rate     = 0.05,   # step size: smaller = more robust, needs more trees
    subsample         = 0.8,    # fraction of training rows used per tree
    colsample_bytree  = 0.8,    # fraction of features used per tree
    min_child_weight  = 5,      # minimum sum of sample weights in a leaf (like min_samples_leaf)
    reg_lambda        = 1.0,    # L2 regularisation on leaf weights (reduces overfitting)
    tree_method       = "hist", # fast histogram algorithm — important for large datasets
    n_jobs            = -1,
    random_state      = 42,
)
...
out_dir    = Path(f"netcdf/{site}_filled_xgb/")
...
for nc_file in sorted(nc_files):
    ...
    scf_flat = scf_yr.reshape(len(times_yr), -1)
...
val_df.to_csv(csv_dir / "xgb_validation_stats.csv", index=False)
print(f"\nSaved: {csv_dir}/xgb_validation_stats.csv")
...
fig.suptitle(f"{site.capitalize()}: XGBoost validation", fontsize=12)
fig.tight_layout()
fig.savefig(fig_dir / "xgb_validation_scatter.png", dpi=150)
print("Saved scatter plot")

# RMSE by elevation band (elevation holdout — most informative)
...
ax2.legend()
fig2.tight_layout()
fig2.savefig(fig_dir / "xgb_validation_rmse_by_elevation.png", dpi=150)
print("Saved RMSE-by-elevation plot")

# ── feature importance ────────────────────────────────────────────────────────
# XGBoost reports gain-based importance: how much each feature reduces the loss
# on average across all splits where it is used (more interpretable than RF's
# impurity-based importance, which tends to favour high-cardinality features).
print("\nTraining final model on all land pixels...")
X_final, y_final = subsample(X_all, y_all, MAX_TRAIN_SAMPLES)
xgb_final = XGBRegressor(**XGB_PARAMS)
xgb_final.fit(X_final, y_final, verbose=False)

imp = pd.Series(xgb_final.feature_importances_, index=FEAT_NAMES).sort_values(ascending=False)
...
for nc_file in sorted(nc_files):
    ...
    scf_np      = da_yr.values.astype(np.float32).copy()
    ...
    for i, (doy, year) in enumerate(zip(doys_yr, years_yr)):
        ...
        pred = xgb_final.predict(X_ice).clip(0, 100).astype(np.float32)
    ...
    ds_out["scf_filled"].attrs = {
        "units":     "%",
        "long_name": "Snow cover fraction — land=observed, glacier=XGBoost predicted",
    }
    ds_out["fill_flag"].attrs = {
        "long_name": "Fill source",
        "flag_values": "0, 1",
        "flag_meanings": "observed XGBoost_predicted",
    }
    ds_out.attrs["source"]              = f"MODIS MOD10A1F + XGBoost glacier infill; site={site}"
    ds_out.attrs["xgb_r2_random"]       = f"{r2_r:.3f}"
    ds_out.attrs["xgb_r2_elev"]         = f"{r2_e:.3f}"
    ds_out.attrs["xgb_rmse_elev"]       = f"{rmse_e:.2f} %"
    ds_out.attrs["min_land_obs_frac"]   = (
        f"{MIN_LAND_OBS_FRAC} (glacier fill suppressed below this fraction of valid land pixels)"
    )

    out_path = out_dir / f"{site}_scf_{year_val}_filled_xgb.nc"
